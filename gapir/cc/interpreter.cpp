...
Interpreter::Result Interpreter::pushI(uint32_t opcode) {
  ...
  switch (type) {
    ...
    case BaseType::Int32:
      data |= (data & 0x80000) != 0 ? 0xfff00000ULL : 0x0; // Sign extend the 20 bit immediate value
      mStack.push(*((int32_t*)&data));
    break;

    case BaseType::Int64:
      data |= (data & 0x80000) != 0 ? 0xfffffffffff00000ULL : 0x0; // Sign extend the 20 bit immediate value
      mStack.push(*((int64_t*)&data));
    break;
    ...
    case BaseType::Float:
      data <<= 23; // Shifting the value into the exponent for floating point types
      mStack.push(*((float*)&data));
    break;

    case BaseType::Double:
      data <<= 52; // Shifting the value into the exponent for floating point types
      mStack.push(*((double*)&data));
    break;

    case BaseType::AbsolutePointer:
      mStack.push(*((void**)&data));
    break;
    ...
    default:
      assert(false); // Unhandled type in pushI
    break;
  }
...
Interpreter::Result Interpreter::loadC(uint32_t opcode) {
  ...
  switch (type) {
    ...
    default:
      assert(false); // Unhandled type in loadC
    break;
  }
...
Interpreter::Result Interpreter::store(uint32_t) {
...
Interpreter::Result Interpreter::post(uint32_t) {
  return this->call(Interpreter::POST_FUNCTION_ID);
}

Interpreter::Result Interpreter::notification(uint32_t) {
  ...
  if(type == typeid(Stack::VolatilePointer)) {
    ...
    if(sizeof(void*) == 8) {
      ...
      *ptr = (*ptr << 26) | data;
      ...
      assert(false); // This system doesn't have 8 or 4 byte pointers?
    }
    mStack.push(val);
    success = true;
  }
    ...
    *ptr |= exponent;
    ...
    assert(false); // Don't know how to extend this type.
  }

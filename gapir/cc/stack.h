...
class Stack {
 public:
  ...
  template <typename T>
  void push(T value) {
    assert(mNumEntries < mMaxEntries);
    mStack[mNumEntries++].set(value);
  }

  void discard(uint32_t count) { assert(mNumEntries-- > 0); }

  void clone(uint32_t n) {
    assert(mNumEntries > n);
  ...
  class Entry {
   public:
    ...
    const T& get() {
      assertCanStore<T>();
      #ifndef NDEBUG
      // We only check the typeid for get() in debug builds. The rest of the time we just
      // make sure the number of bytes is correct.
      assert (sizeof(T) == mDataSize && mDataType == typeid(T));
      #else
    ...
    void copyTo(void* address) { memcpy(address, mData, mDataSize); }

    Entry& operator=(const Entry& rhs) {
      this->mDataSize = rhs.mDataSize;
      memcpy(mData, rhs.mData, maxEntrySize);
      return *this;
    }

   private:
    constexpr static const size_t maxEntrySize = 16;
    char mData[maxEntrySize];
